define([ 'jquery', 'lodash', 'tabhelper', 'moment', 'utils/utils', 'utils/pagination' ],
    function($, _, tabHelper, moment, utils, Pagination) {
        ...
        $('[role=add]').on('click', function() {
            tabHelper.openChildTab('/auth/role/add')
        })

        //跳转到编辑页面
        $list.on('click', '[role=edit]', function () {
            var id = $(this).data('id')
            tabHelper.openChildTab('/auth/role/edit?id=' + id)
        })
        ...
        $list.on('click', '[role=delete]', function() {
            ...
            if (ok) {
                ...
                $.ajax({
                    type : 'get',
                    dataType : 'json',
                    url : '/auth/role/proto/delete?id='+id,
                }).done(function(result) {
                    if (result.status === 'ok') {
                        pager.reload()
                    } else {
        ...
        $list.on('click', '[role=assign]', function () {
            var id = $(this).data('id')
            tabHelper.openChildTab('/auth/role/assign?id=' + id)
        })
