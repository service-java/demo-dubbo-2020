define([ 'jquery', 'lodash', 'tabhelper', 'moment', 'utils/utils', 'utils/pagination' ], 
	...
	$('[role=add]').on('click', function() {
		tabHelper.openChildTab('/gateway/apigroup/add')
	})

	//跳转到编辑页面
	$list.on('click', '[role=edit]', function () {
		var id = $(this).data('id')
		tabHelper.openChildTab('/gateway/apigroup/edit?id=' + id)
	})
	
	//跳转到分组内页
	$list.on('click', '[role=detail]', function () {
		var id = $(this).data('id')
		tabHelper.openChildTab('/gateway/apigroup/detail?id=' + id)
	})
	...
	$list.on('click', '[role=delete]', function() {
		...
		if (ok) {
			...
			$.ajax({
				type : 'get',
				dataType : 'json',
				url : '/gateway/apigroup/delete?id='+id,
			}).done(function(result) {
